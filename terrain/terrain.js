...
document.onkeydown = (ev) => {
	switch (ev.code) {
		case 'KeyI': {
			cameraPosition = add(cameraPosition, directionOfCamera); // zoom in
			updateCamera();
			break;
		}
		case 'KeyO': {
			cameraPosition = subtract(cameraPosition, directionOfCamera); // zoom out
			updateCamera();
			break;
		}
		case 'ArrowLeft': {
			cameraPosition = subtract(cameraPosition, [-directionOfCamera[2], 0, directionOfCamera[0]]); // left
			updateCamera();
			break;
		}
		case 'ArrowRight': {
			cameraPosition = add(cameraPosition, [-directionOfCamera[2], 0, directionOfCamera[0]]); // right
			updateCamera();
			break;
		}
		case 'ArrowUp': {
			cameraPosition  = add(cameraPosition, [directionOfCamera[0], 0, Math.cos(cameraPhi)]); // forward
			updateCamera();
			break;
		}
		case 'ArrowDown': {
			cameraPosition = subtract(cameraPosition, [directionOfCamera[0], 0, Math.cos(cameraPhi)]); // backward
			updateCamera();
			break;
		}
		case 'KeyU': {
			cameraPosition[1] += 1; // up
			updateCamera();
			break;
		}
		case 'KeyD': {
			cameraPosition[1] -= 1; // down
			updateCamera();
			break;
		}
		default: return;
